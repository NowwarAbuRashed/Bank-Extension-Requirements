...
void ShowMenueScreen()
{
    ...
    cout << Taps(1) << "[6] Transactions.\n";
    cout << Taps(1) << "[7] Exit.\n";
    cout << "===========================================\n";
    cout << "Choose what do you want to do? [1 to 7]?";
}
void ShowMenueTransactionsScreen()
{
    system("cls");

    cout << "===========================================\n";
    cout << Taps(2) << "Transactions Menue Screen\n";
    cout << "===========================================\n\n";
    cout << Taps(1) << "[1] Deposit.\n";
    cout << Taps(1) << "[2] Witedraw.\n";
    cout << Taps(1) << "[3] Total Balances.\n";
    cout << Taps(1) << "[4] Main Menue.\n";
    cout << "===========================================\n";
...
void ShowWitedrawScreen()
{
    cout << "---------------------------------------\n";
    cout << Taps(1) << "Witedraw Screen\n";
    cout << "---------------------------------------\n\n";
...
{
    cout << "---------------------------------------\n";
    cout << Taps(1) << "Total Balances Screen\n";
    cout << "---------------------------------------\n\n";
...
enum enMainMenueOptions
{
    ...
    enUpdateClientInfo = 4, enFindClient = 5,Transactions=6 ,Exit = 7

};
enum enMainMenueTransactions
{
    enDeposit=1,enWithedraw=2,
    enTotalBalances=3,enMainMenue=4
};

void Deposet(vector<sClientData>& vClients)
{
    string Acocunt = ReadString("\nPlease enter AccountNumber? ");
    short indxClient = CheckClientIsFound(vClients, Acocunt);
    while (indxClient==-1)
    {
        cout << "Client with [" << Acocunt << "] does no exist.\n";
        Acocunt = ReadString("\nPlease enter AccountNumber? ");
        indxClient = CheckClientIsFound(vClients, Acocunt);
    }
    cout << "\nThe following are the client details:\n";
    cout << "--------------------------------------\n";
    PrintRecord(vClients[indxClient]);
    ...
    double deposet = ReadPositeveNumberDouble("Please enter deposit amount ? ");

    cout << "Are you sure you want perform this transaction? y/n ?";
    char AddDeposet = 'y';
    cin >> AddDeposet;
    if (tolower(AddDeposet)=='y')
    {
        vClients[indxClient].AccountBalance += deposet;
        AddDataLineToFile(ClientsFileName, vClients);
    }
...
void PrintLineRecordBalances(sClientData ClientData)
{
    cout << "| " << left << setw(15) << ClientData.AccountNumber;
...
void PrintAllClientsDataBalances(vector<sClientData> &vClientData)
{
    system("cls");
    cout << Taps(5) << "Balances Lint (" << vClientData.size() << ") Client(s).\n";
    cout << "\n____________________________________________________________________________________________________\n\n";
    ...
    for (sClientData &ClientData : vClientData)
    {
        PrintLineRecordBalances(ClientData);
        cout << endl;
    }
    cout << "\n____________________________________________________________________________________________________\n\n";
...
void TotalBalances(vector<sClientData>& vClientData)
{
    PrintAllClientsDataBalances(vClientData);
    double Total = 0;
    for (sClientData &ClientData : vClientData)
    {
        Total += ClientData.AccountBalance;
    }
    cout<<Taps(4) << "Total Balance = " << Total << "\n\n";
}

void Withedraw(vector<sClientData>& vClients)
{
    string Acocunt = ReadString("\nPlease enter AccountNumber? ");
    short indxClient = CheckClientIsFound(vClients, Acocunt);
    while (indxClient == -1)
    ...
    double deposet = ReadPositeveNumberDouble("Please enter withdraw amount ? ");
    while (deposet > vClients[indxClient].AccountBalance)
    {
        cout << "Amount Exceeds the balance, you can withdraw up to : " << vClients[indxClient].AccountBalance << endl;
        deposet = ReadPositeveNumberDouble("Please enter withdraw amount ? ");
    }

    cout << "Are you sure you want perform this transaction? y/n ?";
    char minasDeposet = 'y';
    cin >> minasDeposet;
    if (tolower(minasDeposet) == 'y')
    {
        vClients[indxClient].AccountBalance -= deposet;
        AddDataLineToFile(ClientsFileName, vClients);
    }
...
void TransactionsMenueScreen(vector<sClientData>& vClients)
{
    short NumberChoice = 0;

    while (true)
    {
        ShowMenueTransactionsScreen();
        cin >> NumberChoice;
        switch ((enMainMenueTransactions)NumberChoice)
        {
        case enDeposit:
            system("cls");
            ShowDepositScreen();
        ...
        default:
            cout << "Rong Choose what do you want to do? [1 to 4]?";
            break;
        }
        system("pause");
...
void StartBank()
{
    ...
    while (true)
    {
        ...
        case enMainMenueOptions::Transactions:
            system("cls");
            TransactionsMenueScreen(ClientData);
            break;
        case enMainMenueOptions::Exit:
            ...
            break;
        default:
            cout << "Rong Choose what do you want to do? [1 to 7]?";
            break;
        }
